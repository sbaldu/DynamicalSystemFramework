...
///             The Dynamics class represents the dynamics of the network. It is
///             templated by the type of the graph's id and the type of the
///             graph's capacity. The graph's id and capacity must be unsigned
///             integral types.
...
/// @brief The Measurement struct represents the mean of a quantity and its
/// standard deviation
/// @param mean The mean
...
struct Measurement {
  ...
  Measurement(const std::vector<T> &data) {
    ...
    std::for_each(data.begin(), data.end(), 0.,
                  [&x_mean, &x2_mean](auto value) -> void {
                    x_mean += value;
                    x2_mean += value * value;
                  });
...
/// @tparam Id, The type of the graph's id. It must be an unsigned integral
/// type.
/// @tparam Size, The type of the graph's capacity. It must be an unsigned
/// integral type.
...
protected:
  ...
  /// @param reinsert_agents If true, the agents are reinserted in the
  /// simulation after they reach their destination
  /// @details If possible, removes the first agent of each street queue,
  /// putting it in the destination node. If the agent is going into the
  /// destination node, it is removed from the simulation (and then reinserted
  /// if reinsert_agents is true)
  virtual void m_evolveStreets(bool reinsert_agents);
  /// @brief Evolve the nodes
  /// @details If possible, removes all agents from each node, putting them on
  /// the next street. If the error probability is not zero, the agents can move
  /// to a random street.
  virtual void m_evolveNodes();
  /// @brief Evolve the agents.
  /// @details Puts all new agents on a street, if possible, decrements all
  /// delays and increments all travel times.
...
public:
  ...
  /// @brief Set the minim speed rateo, i.e. the minim speed with respect to the
  /// speed limit
  /// @param minSpeedRateo The minim speed rateo
  /// @throw std::invalid_argument If the minim speed rateo is not between 0 and
  /// 1
  void setMinSpeedRateo(double minSpeedRateo);
  /// @brief Set the error probability
  /// @param errorProbability The error probability
  /// @throw std::invalid_argument If the error probability is not between 0 and
  /// 1
  void setErrorProbability(double errorProbability);
  /// @brief Set the maximum flow percentage
  /// @param maxFlowPercentage The maximum flow percentage
  /// @details The maximum flow percentage is the percentage of the maximum flow
  /// that a street can transmit. Default is 1 (100%).
  /// @throw std::invalid_argument If the maximum flow percentage is not between
  /// 0 and 1
  void setMaxFlowPercentage(double maxFlowPercentage);
  /// @brief Set the speed of an agent
  /// @details This is a pure-virtual function, it must be implemented in the
  /// derived classes
  /// @param agentId The id of the agent
  virtual void setAgentSpeed(Size agentId) = 0;
  /// @brief Set the force priorities flag
  /// @param forcePriorities The flag
  /// @details If true, if an agent cannot move to the next street, the whole
  /// node is skipped
  ...
  /// @details Some data, i.e. the street queue lengths, are stored only after a
  /// fixed amount of time which is represented by this variable.
  void setDataUpdatePeriod(Delay dataUpdatePeriod) {
    m_dataUpdatePeriod = dataUpdatePeriod;
  }

  /// @brief Update the paths of the itineraries based on the actual travel
  /// times
  virtual void updatePaths();
  /// @brief Evolve the simulation
  /// @details Evolve the simulation by moving the agents and updating the
  /// travel times. In particular:
  /// - Move the first agent of each street queue, if possible, putting it in
  /// the next node
  /// - Move the agents from each node, if possible, putting them in the next
  /// street and giving them a speed. If the error probability is not zero, the
  /// agents can move to a random street. If the agent is in the destination
  /// node, it is removed from the simulation (and then reinserted if
  /// reinsert_agents is true)
  ...
  /// @param nCycles Delay, The number of cycles (times agents are being added)
  /// between two calls of this function
  /// @param threshold double, The percentage of the mean capacity of the
  /// streets used as threshold for the delta between the two tails.
  /// @param densityTolerance double, The algorithm will consider all streets
  /// with density up to densityTolerance*meanDensity
  /// @details The function cycles over the traffic lights and, if the
  /// difference between the two tails is greater than
  ///   the threshold multiplied by the mean capacity of the streets, it changes
  ///   the green and red times of the traffic light, keeping the total cycle
  ///   time constant.
  void optimizeTrafficLights(Delay nCycles, double threshold = 0.,
  ...
  /// @throws std::invalid_argument If the source node or the itinerary are not
  /// found
  ...
  void addAgentsRandomly(Size nAgents, const TContainer &src_weights,
  ...
  /// @param itineraries Generic container of itineraries, represented by an
  /// std::span
  ...
  /// @return Measurement<double> The mean speed of the agents and the standard
  /// deviation
  Measurement<double> agentMeanSpeed() const;
  // TODO: implement the following functions
  // We can implement the base version of these functions by cycling over
  // agents... I won't do it for now. Grufoony - 19/02/2024
  ...
  /// @return Measurement<double> The mean density of the streets and the
  /// standard deviation
  Measurement<double> streetMeanDensity() const;
  /// @brief Get the mean flow of the streets in \f$s^{-1}\f$
  /// @return Measurement<double> The mean flow of the streets and the standard
  /// deviation
  Measurement<double> streetMeanFlow() const;
  /// @brief Get the mean flow of the streets in \f$s^{-1}\f$
  /// @param threshold The density threshold to consider
  /// @param above If true, the function returns the mean flow of the streets
  ...
  /// @param resetValue If true, the spire input/output flows are cleared after
  /// the computation
  /// @return Measurement<double> The mean spire input flow of the streets and
  /// the standard deviation
  /// @details The spire input flow is computed as the sum of counts over the
  /// product of the number of spires and the time delta
  Measurement<double> meanSpireInputFlow(bool resetValue = true);
  /// @brief Get the mean spire output flow of the streets in \f$s^{-1}\f$
  /// @param resetValue If true, the spire output/input flows are cleared after
  /// the computation
  /// @return Measurement<double> The mean spire output flow of the streets and
  /// the standard deviation
  /// @details The spire output flow is computed as the sum of counts over the
  /// product of the number of spires and the time delta
  Measurement<double> meanSpireOutputFlow(bool resetValue = true);
  /// @brief Get the mean travel time of the agents in \f$s\f$
  /// @param clearData If true, the travel times are cleared after the
  /// computation
  /// @return Measurement<double> The mean travel time of the agents and the
  /// standard
  Measurement<double> meanTravelTime(bool clearData = false);
  /// @brief Get the turn counts of the agents
  /// @return const std::array<unsigned long long, 3>& The turn counts
  /// @details The array contains the counts of left (0), straight (1), right
  /// (2) and U (3) turns
  const std::unordered_map<Id, std::array<unsigned long long, 4>> &
  turnCounts() const {
    return m_turnCounts;
  }
  /// @brief Get the turn probabilities of the agents
  /// @return std::array<double, 3> The turn probabilities
  /// @details The array contains the probabilities of left (0), straight (1),
  /// right (2) and U (3) turns
  std::unordered_map<Id, std::array<double, 4>>
  turnProbabilities(bool reset = true);
...
Dynamics<Id, Size, Delay>::Dynamics(Graph<Id, Size> &graph)
    : m_time{0}, m_previousSpireTime{0}, m_graph{std::move(graph)},
      m_errorProbability{0.}, m_minSpeedRateo{0.}, m_maxFlowPercentage{1.},
      m_forcePriorities{false} {
  for (const auto &[streetId, street] : m_graph.streetSet()) {
    m_streetTails.emplace(streetId, 0);
    m_turnCounts.emplace(streetId,
                         std::array<unsigned long long, 4>{0, 0, 0, 0});
    // fill turn mapping as [streetId, [left street Id, straight street Id,
    // right street Id, U self street Id]]
...
Id Dynamics<Id, Size, Delay>::m_nextStreetId(Id agentId, Id nodeId,
  ...
  if (this->m_itineraries.size() > 0 &&
    ...
    const auto &it =
        this->m_itineraries[this->m_agents[agentId]->itineraryId()];
  ...
  } while (!m_graph.nodeSet().at(nodeId)->isRoundabout() and
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::m_increaseTurnCounts(Id streetId,
                                                     double delta) {
  if (std::abs(delta) < std::numbers::pi) {
    if (delta < 0.) {
...
void Dynamics<Id, Size, Delay>::m_evolveStreets(bool reinsert_agents) {
  for (const auto &[streetId, street] : m_graph.streetSet()) {
    ...
    const auto &destinationNode{
        this->m_graph.nodeSet()[street->nodePair().second]};
    if (destinationNode->isFull()) {
      continue;
    }
    if (destinationNode->isTrafficLight()) {
      auto &tl =
          dynamic_cast<TrafficLight<Id, Size, Delay> &>(*destinationNode);
    ...
    if (node->isIntersection()) {
      ...
      for (const auto [angle, agentId] : intersection.agents()) {
        const auto &nextStreet{
            m_graph.streetSet()[m_agentNextStreetId[agentId]]};
        if (!(nextStreet->isFull())) {
      ...
      for (size_t i{0}; i < nAgents; ++i) {
        ...
        if (!(nextStreet->isFull())) {
          if (m_agents[agentId]->streetId().has_value()) {
            ...
            auto delta =
                nextStreet->angle() - m_graph.streetSet()[streetId]->angle();
            if (delta > std::numbers::pi) {
              delta -= 2 * std::numbers::pi;
            } else if (delta < -std::numbers::pi) {
              delta += 2 * std::numbers::pi;
  ...
  for (const auto &[nodeId, node] : m_graph.nodeSet()) {
    ...
    for (const auto [nextNodeId, _] : m_graph.adjMatrix().getRow(nodeId)) {
      if (nextNodeId == destinationID &&
          minDistance == m_graph.streetSet()
                             .at(nodeId * dimension + nextNodeId)
                             ->length()) {
        path.insert(nodeId, nextNodeId, true);
        continue;
      }
      // TimePoint expectedTravelTime{
      //     streetLength};  // / street->maxSpeed()};  // TODO: change into
      //     input speed
      ...
      if (result.has_value()) {
        ...
        if (minDistance ==
            result.value().distance() + m_graph.streetSet()
                                            .at(nodeId * dimension + nextNodeId)
                                            ->length()) {
          path.insert(nodeId, nextNodeId, true);
        }
  ...
  if (path.size() == 0) {
    throw std::runtime_error(buildLog(
        std::format("Path with id {} and destination {} is empty. Please "
                    ...
                    pItinerary->id(), pItinerary->destination())));
  }
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::setItineraries(
    std::span<Itinerary<Id>> itineraries) {
...
void Dynamics<Id, Size, Delay>::setMinSpeedRateo(double minSpeedRateo) {
  if (minSpeedRateo < 0. || minSpeedRateo > 1.) {
    throw std::invalid_argument(buildLog(
        std::format("The minimum speed rateo ({}) must be between 0 and 1",
                    minSpeedRateo)));
  }
...
void Dynamics<Id, Size, Delay>::setErrorProbability(double errorProbability) {
  if (errorProbability < 0. || errorProbability > 1.) {
    throw std::invalid_argument(buildLog(
        std::format("The error probability ({}) must be between 0 and 1",
                    errorProbability)));
  }
...
void Dynamics<Id, Size, Delay>::setMaxFlowPercentage(double maxFlowPercentage) {
  if (maxFlowPercentage < 0. || maxFlowPercentage > 1.) {
    throw std::invalid_argument(buildLog(
        std::format("The maximum flow percentage ({}) must be between 0 and 1",
                    maxFlowPercentage)));
  }
...
void Dynamics<Id, Size, Delay>::evolve(bool reinsert_agents) {
  // move the first agent of each street queue, if possible, putting it in the
  // next node
...
void Dynamics<Id, Size, Delay>::optimizeTrafficLights(Delay nCycles,
  ...
  if (threshold < 0 || threshold > 1) {
    throw std::invalid_argument(buildLog(
        std::format("The threshold parameter is a percentage and must be "
                    "bounded between 0-1. Inserted value: {}",
                    threshold)));
  }
  if (densityTolerance < 0 || densityTolerance > 1) {
    throw std::invalid_argument(buildLog(std::format(
        "The densityTolerance parameter is a percentage and must be "
        "bounded between 0-1. Inserted value: {}",
        densityTolerance)));
  }
  ...
  for (const auto &[nodeId, node] : m_graph.nodeSet()) {
    ...
    const Delay delta = std::floor(
        std::fabs(static_cast<int>(greenQueue - redQueue)) / nCycles);
    if (delta == 0) {
      continue;
    }
    ...
    //    - Check that the incoming streets have a density less than the mean
    //    one (eventually + tolerance): I want to avoid being into the cluster,
    //    better to be out or on the border
    ...
    // std::cout << '\t' << " -> Mean network density: " << std::setprecision(7)
    // << meanDensityGlob << '\n'; std::cout << '\t' << " -> Mean density of 4
    // outgoing streets: " << std::setprecision(7) << meanDensity_streets <<
    // '\n';
    ...
    // std::cout << '\t' << " -> Parametro ratio: " << std::setprecision(7) <<
    // ratio << '\n'; std::cout << '\t' << " -> Parametro dyn_thresh: " <<
    // std::setprecision(7) << dyn_thresh << '\n';
    if (meanDensityGlob * (1. + dyn_thresh) > meanDensity_streets) {
      ...
      if (meanDensityGlob > meanDensity_streets) {
        ...
        // std::cout << '\t' << " -> greenTime: " << static_cast<unsigned
        // int>(greenTime) << '\n'; std::cout << '\t' << " -> redTime: " <<
        // static_cast<unsigned int>(redTime) << '\n'; std::cout << '\t' << " ->
        // modTime: " << tl.modTime() << '\n';
      } else {
...
void Dynamics<Id, Size, Delay>::addAgent(const Agent<Id, Size, Delay> &agent) {
  if (this->m_agents.size() + 1 > this->m_graph.maxCapacity()) {
    throw std::overflow_error(buildLog(std::format(
        "Graph its already holding the max possible number of agents ({})",
  ...
  this->m_agents.emplace(agent.id(),
                         std::make_unique<Agent<Id, Size, Delay>>(agent));
}
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::addAgent(
    std::unique_ptr<Agent<Id, Size, Delay>> agent) {
  if (this->m_agents.size() + 1 > this->m_graph.maxCapacity()) {
    throw std::overflow_error(buildLog(std::format(
        "Graph its already holding the max possible number of agents ({})",
        this->m_graph.maxCapacity())));
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::addAgents(
    std::span<Agent<Id, Size, Delay>> agents) {
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::addAgentsUniformly(
    Size nAgents, std::optional<Id> itineraryId) {
  if (this->m_agents.size() + nAgents > this->m_graph.maxCapacity()) {
    throw std::overflow_error(buildLog(std::format(
        "Graph its already holding the max possible number of agents ({})",
        this->m_graph.maxCapacity())));
  ...
  for (Size i{0}; i < nAgents; ++i) {
    ...
    Agent<Id, Size, Delay> agent{agentId, itineraryId.value(),
...
template <typename TContainer>
  requires(std::is_same_v<TContainer, std::unordered_map<Id, double>> ||
           std::is_same_v<TContainer, std::map<Id, double>>)
void Dynamics<Id, Size, Delay>::addAgentsRandomly(
    Size nAgents, const TContainer &src_weights,
    const TContainer &dst_weights) {
  // Check if the weights are normalized
  if (std::abs(std::accumulate(src_weights.begin(), src_weights.end(), 0.,
  ...
  if (std::abs(std::accumulate(dst_weights.begin(), dst_weights.end(), 0.,
                               [](double sum, const std::pair<Id, double> &p) {
                                 return sum + p.second;
                               }) -
               1.) > std::numeric_limits<double>::epsilon()) {
  ...
  while (nAgents > 0) {
    ...
    auto itineraryIt{std::find_if(m_itineraries.begin(), m_itineraries.end(),
    ...
    if (itineraryIt == m_itineraries.end()) {
      throw std::invalid_argument(buildLog(
          std::format("Itinerary with destination {} not found.", dstId)));
    }
...
void Dynamics<Id, Size, Delay>::addItinerary(const Itinerary<Id> &itinerary) {
  m_itineraries.emplace(itinerary.id(),
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::addItinerary(
    std::unique_ptr<Itinerary<Id>> itinerary) {
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
void Dynamics<Id, Size, Delay>::addItineraries(
    std::span<Itinerary<Id>> itineraries) {
...
Measurement<double> Dynamics<Id, Size, Delay>::agentMeanSpeed() const {
  ...
  const double mean{std::accumulate(m_agents.cbegin(), m_agents.cend(), 0.,
  ...
  const double variance{
      std::accumulate(m_agents.cbegin(), m_agents.cend(), 0.,
...
Measurement<double> Dynamics<Id, Size, Delay>::streetMeanDensity() const {
  ...
  const double variance{
      std::accumulate(
          m_graph.streetSet().cbegin(), m_graph.streetSet().cend(), 0.,
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
Measurement<double>
Dynamics<Id, Size, Delay>::streetMeanFlow(double threshold, bool above) const {
  std::vector<double> flows;
  flows.reserve(m_graph.streetSet().size());
  for (const auto &[streetId, street] : m_graph.streetSet()) {
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
Measurement<double>
Dynamics<Id, Size, Delay>::meanSpireInputFlow(bool resetValue) {
  ...
  for (const auto &[streetId, street] : m_graph.streetSet()) {
    if (street->isSpire()) {
      ...
      flows.push_back(static_cast<double>(spire.inputCounts(resetValue)) /
                      deltaTime);
    }
  }
  return Measurement(flows);
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           is_numeric_v<Delay>)
Measurement<double>
Dynamics<Id, Size, Delay>::meanSpireOutputFlow(bool resetValue) {
  auto deltaTime{m_time - m_previousSpireTime};
  if (deltaTime == 0) {
    return Measurement(0., 0.);
  }
  ...
  for (const auto &[streetId, street] : m_graph.streetSet()) {
    if (street->isSpire()) {
      auto &spire = dynamic_cast<SpireStreet<Id, Size> &>(*street);
      flows.push_back(static_cast<double>(spire.outputCounts(resetValue)) /
                      deltaTime);
    }
  }
  return Measurement(flows);
...
Measurement<double> Dynamics<Id, Size, Delay>::meanTravelTime(bool clearData) {
  ...
  const double mean{
      std::accumulate(m_travelTimes.cbegin(), m_travelTimes.cend(), 0.) /
      m_travelTimes.size()};
  const double variance{
      std::accumulate(m_travelTimes.cbegin(), m_travelTimes.cend(), 0.,
...
public:
  ...
  /// @return double The mean speed of the street or street->maxSpeed() if the
  /// street is empty
  /// @details The mean speed of a street is given by the formula:
  /// \f$ v_{\text{mean}} = v_{\text{max}} \left(1 - \frac{\alpha}{2} \left( n -
  /// 1\right)  \right) \f$ where \f$ v_{\text{max}} \f$ is the maximum speed of
  /// the street, \f$ \alpha \f$ is the minimum speed rateo divided by the
  /// capacity and \f$ n \f$ is the number of agents in the street
  double streetMeanSpeed(Id streetId) const override;
  /// @brief Get the mean speed of the streets in \f$m/s\f$
  /// @return Measurement The mean speed of the agents and the standard
  /// deviation
  Measurement<double> streetMeanSpeed() const override;
  /// @brief Get the mean speed of the streets with density above or below a
  /// threshold in \f$m/s\f$
  /// @param threshold The density threshold to consider
  /// @param above If true, the function returns the mean speed of the streets
  /// with a density above the threshold, otherwise below
  /// @return Measurement The mean speed of the agents and the standard
  /// deviation
  Measurement<double> streetMeanSpeed(double threshold,
...
void FirstOrderDynamics<Id, Size, Delay>::setAgentSpeed(Size agentId) {
  ...
  if (this->m_speedFluctuationSTD > 0.) {
    std::normal_distribution<double> speedDist{
        speed, speed * this->m_speedFluctuationSTD};
    speed = speedDist(this->m_generator);
  }
  speed < 0.
      ? agent->setSpeed(street->maxSpeed() * (1. - this->m_minSpeedRateo))
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           std::unsigned_integral<Delay>)
Measurement<double>
FirstOrderDynamics<Id, Size, Delay>::streetMeanSpeed() const {
...
template <typename Id, typename Size, typename Delay>
  requires(std::unsigned_integral<Id> && std::unsigned_integral<Size> &&
           std::unsigned_integral<Delay>)
Measurement<double>
FirstOrderDynamics<Id, Size, Delay>::streetMeanSpeed(double threshold,
                                                     bool above) const {
  if (this->m_agents.size() == 0) {
    return Measurement(0., 0.);
  }
