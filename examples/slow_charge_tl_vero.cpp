...
// #define PRINT_SPEEDS
// #define PRINT_TP
// TODO questo è il branch con il nuovo ALGORITMO
//#define OPTIMIZE
...
const std::string IN_MATRIX{"./matrix.dat"}; // input matrix file
const std::string IN_COORDS{"./coordinates.dsm"}; // input coords file
// TODO nuova cartella output
//const std::string OUT_FOLDER{"./output-tesi/greg-op_seed" + std::to_string(SEED) + "_" + std::to_string(nAgents) + "_gaussiana_m100_std20/"}; // output folder
const std::string OUT_FOLDER{"./output-tesi/no-op_seed" + std::to_string(SEED) + "_" + std::to_string(nAgents) + "_gaussiana_m60_std10/"}; // output folder
//const std::string OUT_FOLDER{"./output-tesi/new-op_dyn-vrs-copy2-oi2_seed" + std::to_string(SEED) + "_" + std::to_string(nAgents) + "_gaussiana_m100_std20/"}; // output folder
...
int main() {
  ...
  // TODO cambia i parametri dei semafori
  const auto meanTime = 60.;
  const auto stdTime = 10.;
  ...
  std::mt19937 gen(rd());
  gen.seed(64313);
  std::normal_distribution d(meanTime, stdTime);
...
#endif
#ifdef OPTIMIZE
  std::ofstream modTime(OUT_FOLDER + "modTimes.csv");
  std::ofstream tl_counter(OUT_FOLDER + "tl_counter.csv");
    modTime << "time;";
    for (const auto &[nodeId, node] : dynamics.graph().nodeSet()) {
      modTime << nodeId << ';';
    }
    modTime << '\n';
...
#ifdef OPTIMIZE
    if (dynamics.time() % 420 == 0) { //420
      std::cout << '\t' << "-> Chiamata alla funzione optimizeTrafficLights" <<'\n';
      dynamics.optimizeTrafficLights(std::floor(420. / 60), 0.15);
      modTime << dynamics.time() << ';';
      tl_counter << dynamics.time() << ';';
      int counter{0};
      for (const auto &[nodeId, node] : dynamics.graph().nodeSet()) {
        auto &tl = dynamic_cast<TrafficLight &>(*node);
        modTime << tl.modTime() << ';';
